...
public class TBCS_ModificaReferenciaSistema extends HttpServlet implements SingleThreadModel{
...
private String v_nuevaCadena ="";
  ...
  v_Consulta.TBPL_RealizarConexion();//Realiza la conexion con la base de datos
    ...
    /*try{
      ...
      try{
            CleanResults cr1 = as.scan(request.getParameter("v_codigo"), new File(POLICY_FILE_LOCATION));
            ...
            CleanResults cr3 = as.scan(request.getParameter("v_ddescripcion"), new File(POLICY_FILE_LOCATION));
            v_ddescripcion = cr3.getCleanHTML();
            CleanResults cr4 = as.scan(request.getParameter("v_valor"), new File(POLICY_FILE_LOCATION));
            v_valor = cr4.getCleanHTML(); 
            CleanResults cr5 = as.scan(request.getParameter("v_dvalor"), new File(POLICY_FILE_LOCATION));
            v_dvalor = cr5.getCleanHTML(); 
            }
         catch (Exception e) { e.printStackTrace(); }//INT20131108
 ...
 if ((v_codigo!=null)&&(v_descripcion!=null))
  ...
  out.println(STBCL_GenerarBaseHTML.TBFL_CABEZA("Administración de referencias de sistema","Administración de referencias de sistema","", "<BLOCKQUOTE><BR>"+resultado.elementAt(0).toString()+"</BLOCKQUOTE>", false, "moduloparametro.js", "return checkrequired(this)"));
  ...
  public String getServletInfo() {
    return "TBPKT_REFERENCIASISTEMA.TBCS_ModificaReferenciaSistema Information";
  }
}
