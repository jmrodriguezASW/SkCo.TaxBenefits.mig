...
public class i_retiro
extends sqlj.runtime.ref.ResultSetIterImpl
implements sqlj.runtime.NamedIterator
{
  public i_retiro(sqlj.runtime.profile.RTResultSet resultSet)
    throws java.sql.SQLException
  {
    ...
    RET_VALORNdx = findColumn("RET_VALOR");
  ...
  public double RET_VALOR()
    throws java.sql.SQLException
  {
    double __sJtmp = m_rs.getDouble(RET_VALORNdx);
    if (m_rs.wasNull()) throw new sqlj.runtime.SQLNullException(); else return __sJtmp;
  }
  private int RET_VALORNdx;
...
/*@lineinfo:user-code*//*@lineinfo:18^221*/

 /**Variable tipo iterator datos retiro*/
 i_retiro v_retiro;

 /**Procedimiento local que consulta a la base de datos la información del retiro a eliminar.*/
 public void TBPL_Eliminar(PrintWriter out,HttpSession session,HttpServletRequest request,String nuevaCadena )
 {
  ...
  try
  {
   ...
   boolean v_nopintar  = true;/**Validar que al tomar el primer dato se coja información del contrato*/

   /**Tipo String*/
   String v_coduni     = "";/** Variable código unidad*/
   ...
   String v_tuni       = "";/** Variable código unidad as400*/
   String v_pintar     = "";/** Dibujar cabeza página de respuesta*/
   String v_pie        = "";/** Dibujar pie de págibna de respueata*/
   String  v_tipoci    = "";
   String  v_hora      = "";
   String  v_horacierre = "";
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleCall(__sJT_cc,"0TBPKT_MODULO_RETIROS.TBPKT_ELIMINAR_MULTIFUND.TBCL_EliminarMulti",theSqlTS);
   if (__sJT_ec.isNew())
   {
      __sJT_st.registerOutParameter(1,oracle.jdbc.OracleTypes.VARCHAR);
   }
   // set IN parameters
   __sJT_st.setString(2,v_tusu);
  // execute statement
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleCall(__sJT_cc,"1TBPKT_MODULO_RETIROS.TBPKT_ELIMINAR_MULTIFUND.TBCL_EliminarMulti",theSqlTS);
   if (__sJT_ec.isNew())
   {
      __sJT_st.registerOutParameter(1,oracle.jdbc.OracleTypes.VARCHAR);
   }
   // set IN parameters
   __sJT_st.setString(2,v_tuni);
  // execute statement
   __sJT_ec.oracleExecuteUpdate();
   // retrieve OUT parameters
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleCall(__sJT_cc,"2TBPKT_MODULO_RETIROS.TBPKT_ELIMINAR_MULTIFUND.TBCL_EliminarMulti",theSqlTS);
   if (__sJT_ec.isNew())
   {
      __sJT_st.registerOutParameter(1,oracle.jdbc.OracleTypes.VARCHAR);
   }
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleCall(__sJT_cc,"3TBPKT_MODULO_RETIROS.TBPKT_ELIMINAR_MULTIFUND.TBCL_EliminarMulti",theSqlTS);
   if (__sJT_ec.isNew())
   {
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleStatement(__sJT_cc,"4TBPKT_MODULO_RETIROS.TBPKT_ELIMINAR_MULTIFUND.TBCL_EliminarMulti",theSqlTS);
   if (__sJT_ec.isNew())
   {
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleCall(__sJT_cc,"5TBPKT_MODULO_RETIROS.TBPKT_ELIMINAR_MULTIFUND.TBCL_EliminarMulti",theSqlTS);
   if (__sJT_ec.isNew())
   {
      __sJT_st.registerOutParameter(1,oracle.jdbc.OracleTypes.VARCHAR);
   }
  ...
  // execute statement
   __sJT_ec.oracleExecuteUpdate();
   // retrieve OUT parameters
   v_horacierre = (String)__sJT_st.getString(1);
  } finally { __sJT_ec.oracleClose(); }
}
...

{
  ...
  try {
   String theSqlTS = "BEGIN :1 := TB_FFECHA_SIGUIENTE(1) \n; END;";
   __sJT_st = __sJT_ec.prepareOracleCall(__sJT_cc,"6TBPKT_MODULO_RETIROS.TBPKT_ELIMINAR_MULTIFUND.TBCL_EliminarMulti",theSqlTS);
   if (__sJT_ec.isNew())
   {
      __sJT_st.registerOutParameter(1,oracle.jdbc.OracleTypes.DATE);
   }
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleCall(__sJT_cc,"7TBPKT_MODULO_RETIROS.TBPKT_ELIMINAR_MULTIFUND.TBCL_EliminarMulti",theSqlTS);
   if (__sJT_ec.isNew())
   {
      __sJT_st.registerOutParameter(1,oracle.jdbc.OracleTypes.DATE);
   }
...

{
  ...
  sqlj.runtime.ExecutionContext.OracleContext __sJT_ec = ((__sJT_cc.getExecutionContext()==null) ? sqlj.runtime.ExecutionContext.raiseNullExecCtx() : __sJT_cc.getExecutionContext().getOracleContext());
  try {
   String theSqlTS = "SELECT RET_CONSECUTIVO\n                            ,RET_FECHA_PROCESO\n                            ,RET_FECHA_EFECTIVA\n                            ,RET_VALOR\n                            ,RET_VALOR_BRUTO\n                            ,PRC_RETIRO_TOTAL\n                            ,CON_NOMBRES\n                            ,CON_APELLIDOS\n                        FROM tbretiros\n                             ,tbproductos_conceptos\n                             ,tbcontratos\n                       WHERE RET_CON_PRO_CODIGO            =  :1  \n                         AND RET_CON_NUMERO                =  :2  \n                         AND RET_REF_ESTADO                = 'SER005'\n                         AND RET_CON_PRO_CODIGO            = PRC_PRO_CODIGO\n                         AND RET_COT_REF_TIPO_TRANSACCION  = PRC_COT_REF_TIPO_TRANSACCION\n                         AND RET_COT_REF_CLASE_TRANSACCION = PRC_COT_REF_CLASE_TRANSACCION\n                         AND RET_CON_NUMERO                = CON_NUMERO";
   __sJT_st = __sJT_ec.prepareOracleStatement(__sJT_cc,"8TBPKT_MODULO_RETIROS.TBPKT_ELIMINAR_MULTIFUND.TBCL_EliminarMulti",theSqlTS);
   // set IN parameters
   __sJT_st.setString(1,v_pro);
   __sJT_st.setString(2,v_contra);
   // execute query
   v_retiro = new TBPKT_MODULO_RETIROS.TBPKT_ELIMINAR_MULTIFUND.TBCL_EliminarMulti.i_retiro(new sqlj.runtime.ref.OraRTResultSet(__sJT_ec.oracleExecuteQuery(),__sJT_st,"8TBPKT_MODULO_RETIROS.TBPKT_ELIMINAR_MULTIFUND.TBCL_EliminarMulti",null));
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleCall(__sJT_cc,"9TBPKT_MODULO_RETIROS.TBPKT_ELIMINAR_MULTIFUND.TBCL_EliminarMulti",theSqlTS);
   if (__sJT_ec.isNew())
   {
      __sJT_st.registerOutParameter(1,oracle.jdbc.OracleTypes.VARCHAR);
   }
