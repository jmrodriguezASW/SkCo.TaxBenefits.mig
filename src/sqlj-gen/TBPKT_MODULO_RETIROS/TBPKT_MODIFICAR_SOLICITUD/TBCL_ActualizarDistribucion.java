...

{
  ...
  try {
   String theSqlTS = "SELECT TO_CHAR(RET_FECHA_EFECTIVA,'YYYYMMDD')\n                  ,RET_VALOR\n                  ,RET_RETIRO_PRORRATA\n                  ,PRC_RETIRO_TOTAL\n               \n              FROM TBRETIROS\n                   ,TBPRODUCTOS_CONCEPTOS\n             WHERE RET_CON_PRO_CODIGO            =  :1  \n               AND RET_CON_NUMERO                =  :2  \n               AND  RET_CONSECUTIVO              =  :3  \n               AND RET_COT_REF_TRANSACCION      = PRC_COT_REF_TRANSACCION\n               AND RET_COT_REF_TIPO_TRANSACCION = PRC_COT_REF_TIPO_TRANSACCION\n               AND RET_COT_REF_CLASE_TRANSACCION= PRC_COT_REF_CLASE_TRANSACCION";
   __sJT_st = __sJT_ec.prepareOracleStatement(__sJT_cc,"0TBPKT_MODULO_RETIROS.TBPKT_MODIFICAR_SOLICITUD.TBCL_ActualizarDistribucion",theSqlTS);
   if (__sJT_ec.isNew())
   {
   ...
   v_fechaefec = (String)__sJT_rs.getString(1);
   v_valorretiro = __sJT_rs.getDouble(2); if (__sJT_rs.wasNull()) throw new sqlj.runtime.SQLNullException();
   v_prorrata = (String)__sJT_rs.getString(3);
   v_retirototal = (String)__sJT_rs.getString(4);
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleCall(__sJT_cc,"1TBPKT_MODULO_RETIROS.TBPKT_MODIFICAR_SOLICITUD.TBCL_ActualizarDistribucion",theSqlTS);
   if (__sJT_ec.isNew())
   {
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleStatement(__sJT_cc,"2TBPKT_MODULO_RETIROS.TBPKT_MODIFICAR_SOLICITUD.TBCL_ActualizarDistribucion",theSqlTS);
   if (__sJT_ec.isNew())
   {
     __sJT_st.setFetchSize(2);
   }
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleCall(__sJT_cc,"3TBPKT_MODULO_RETIROS.TBPKT_MODIFICAR_SOLICITUD.TBCL_ActualizarDistribucion",theSqlTS);
   if (__sJT_ec.isNew())
   {
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleStatement(__sJT_cc,"4TBPKT_MODULO_RETIROS.TBPKT_MODIFICAR_SOLICITUD.TBCL_ActualizarDistribucion",theSqlTS);
   // set IN parameters
   __sJT_st.setString(1,v_pro);
   __sJT_st.setString(2,v_fonsaldo);
   // execute query
   v_res = new TBPKT_MODULO_RETIROS.TBPKT_MODIFICAR_SOLICITUD.TBCL_ActualizarDistribucion.i_res(new sqlj.runtime.ref.OraRTResultSet(__sJT_ec.oracleExecuteQuery(),__sJT_st,"4TBPKT_MODULO_RETIROS.TBPKT_MODIFICAR_SOLICITUD.TBCL_ActualizarDistribucion",null));
