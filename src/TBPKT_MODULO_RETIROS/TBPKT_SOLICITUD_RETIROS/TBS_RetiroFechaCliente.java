...
public class TBS_RetiroFechaCliente extends HttpServlet {
 ...
 public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
 {
  ...
  try
  {
   ...
   try{session.setAttribute ("s_producto",v_producto);}//request.getParameter("nom_producto"));  }
   catch (Exception e){e.printStackTrace();}
   session.removeAttribute("s_contrato");
   try{ session.setAttribute ("s_contrato",v_contrato);}//request.getParameter("nom_contrato")); }
   catch (Exception e){  e.printStackTrace(); }
   session.removeAttribute("s_usuario");
   try{ session.setAttribute ("s_usuario",v_tipousu);}//request.getParameter("nom_tipousuario")); }
   catch (Exception e) { e.printStackTrace(); }
   session.removeAttribute("s_unidad");
   try{ session.setAttribute ("s_unidad",v_unidad);}//request.getParameter("nom_unidad")); }
   catch (Exception e){e.printStackTrace();}
   session.removeAttribute("s_usuariopipe");
   try{ session.setAttribute ("s_usuariopipe",v_usuario);}//request.getParameter("nom_usuario")); }
   catch (Exception e){ e.printStackTrace(); }
