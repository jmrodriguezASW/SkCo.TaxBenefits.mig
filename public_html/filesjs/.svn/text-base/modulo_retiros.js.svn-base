...
function checkrequired(which) 
{
    ...
    else // Se agregan validaciones campos adicionales OMPEV rballesteros 20160728
    {
        var plazo;
        
        if(document.getElementsByName("v_plazopoliza").length > 0) 
        {
            if(document.getElementsByName("v_plazopoliza").item(0).value != "")
            {
                plazo = Number(document.getElementsByName("v_plazopoliza").item(0).value);
                var min = document.getElementsByName("v_plazomin").item(0).value;
                var max = document.getElementsByName("v_plazomax").item(0).value;
                if(plazo < Number(min) || plazo > Number(max))
                {
                    alert("El plazo de la póliza debe ser de " +min+ " a " +max+ " años");
                    return false;
                }
            }
            else
            {
                alert("Debe ingresar el plazo de la  póliza");
                return false;
            }
        }
        if(document.getElementById("v_causal").length > 0)
        {
            if(document.getElementById("v_causal").selectedIndex == 0)
            {
                alert("Debe seleccionar una causal");
                return false;
            }
        }
        if(document.getElementById("v_despenalizacion").length > 0)
        {
            if(document.getElementById("v_despenalizacion").selectedIndex == 0)
            {
                alert("Debe elegir si o no en aplica descuento");
                return false;
            }
        }
    }
...
function minimo(min,objeto,saldo,neto,tipovalor, montoMinimo, saldoMinimo, vtipoRetiro)
{ 
 var resultado;
 var diferencia;
 var express;
 
    express = String(vtipoRetiro);
   //si es negativo
   if (negativo(objeto))
   {
       return 1;
   ...
   {
     diferencia =  saldo-objeto.value;
    if(express == "1"){
       if(objeto.value < saldo)
       {
           //alert("Retiro no permitido. Si desea utilizar el saldo disponible bruto $"+formatCurrency(saldo)+" le recomendamos realizar un Retiro Parcial.");
           alert("Por favor diligencie un valor menor o igual al disponible sugerido.");
           objeto.focus();
           objeto.select();
           objeto.value ='356';
           objeto.value='0';
           express = '';
           return true;
       }else if(objeto.value > neto){
           alert("Retiro no permitido. El saldo disponible para el contrato son $"+formatCurrency(neto)+" y usted desea retirar $"+formatCurrency(objeto.value)+".");
           objeto.focus();
           objeto.select();
           objeto.value ='356';
           objeto.value='0';
           ...
           return true;
               ...
               if(objeto.value < saldo && objeto.value > neto)
               {
                   alert("Usted ha ingresado un valor cercano al saldo disponible para retiro. En caso de que el retiro no procese por desvalorización de las unidades de los portafolios afectados con el mismo, los recursos entregados serán por el máximo valor disponible en su contrato.");
                   express = '';
                   return true;
               }else if(objeto.value > saldo){
                   alert("Retiro no permitido. El saldo sugerido disponible bruto para el contrato son $"+formatCurrency(saldo)+" y usted desea retirar $"+formatCurrency(objeto.value)+".");
                   objeto.focus();
                   objeto.select();
                   objeto.value ='356';
                   objeto.value='0';
             ...
             {
                if(diferencia<=saldoMinimo) 
                {
                    alert("El valor a retirar implica quedar con un saldo de $"+formatCurrency(diferencia)+" que es menor al saldo mínimo requerido de $"+formatCurrency(saldoMinimo)+". Por favor modifique el valor a retirar para continuar.");
                    objeto.focus();
                    objeto.select();
                    objeto.value ='356';
                    objeto.value='0';
   ...
   if(tipovalor.value == "STV002")//neto
   {
     ...
     {
       if(express == "1"){
           //alert("Retiro no permitido. El Saldo disponible neto para Retiro Express en el contrato son $"+formatCurrency(neto)+" y usted desea retirar $"+formatCurrency(objeto.value)+". Si usted quiere utilizar el Saldo Disponible le recomendamos realizar un Retiro Parcial.");
           alert("Por favor diligencie un valor menor o igual al disponible sugerido.");
           objeto.focus();
           objeto.select();
           objeto.value ='356';
           ...
           return true;
               ...
               if(objeto.value < saldo && objeto.value > neto)
               {
                   ...
                   alert("Retiro no permitido. El saldo sugerido disponible neto para el contrato son $"+formatCurrency(neto)+" y usted desea retirar $"+formatCurrency(objeto.value)+".");
                   objeto.focus();
                   objeto.select();
                   objeto.value ='356';
                   objeto.value='0';
...
//se verifica el valor a retirar
function minimo(min,objeto,saldo,neto,tipovalor, montoMinimo, saldoMinimo)
{ 
 var resultado;
 var diferencia;
   //si es negativo
...
function fechaEfectiva(objetoTipoRetiro, objetoFechaProceso, objetoFechaEfectiva){    /**GAV 20150227 Se procede a agregar una función para que valide las fechas efectiva y proceso al realizar los retiros*/  
  var LongitudCadena = objetoTipoRetiro.length; 
  var FechaEfectiva = objetoTipoRetiro.substring(LongitudCadena, LongitudCadena-10); 
  objetoFechaProceso.value=FechaEfectiva;
  objetoFechaEfectiva.value=FechaEfectiva;
  if(LongitudCadena>10)
  {
     objetoFechaEfectiva.readOnly = false;
  }
  else {
     objetoFechaEfectiva.readOnly = true; 
  }
  return true;
}
...
{ 
   ...
   if(tipovalor.value == "STV002")//neto
   {
     if(objeto.value>neto)
     {
       alert("Retiro no permitido, excede el saldo disponible neto. $"+formatCurrency(neto));
       objeto.focus();
       objeto.select();
       objeto.value ='356';
       objeto.value='0';
   ...
   if(tipovalor.value == "STV001")//bruto
   {
     if(objeto.value>saldo)
     {
       alert("Retiro no permitido, excede el saldo disponible bruto. $"+formatCurrency(saldo));
       objeto.focus();
       objeto.select();
       objeto.value ='356';
       objeto.value='0';
...
//se verifica el valor a retirar
function minimo3(min,objeto,saldo,objeto1,neto,tipovalor,minimocontrato)
{ 
 if(tipovalor.value == "STV002")//neto
   {
       objeto1.select();
       objeto1.value = "$"+formatCurrency(neto);
     if(objeto.value>neto)
     {
       alert("Retiro no permitido, excede el saldo disponible neto. $"+formatCurrency(neto));
       objeto.focus();
   ...
   {
       objeto1.select();
       objeto1.value = "$"+formatCurrency(saldo);
     if(objeto.value>saldo)
     {
       alert("Retiro no permitido, excede el saldo disponible bruto. $"+formatCurrency(saldo));
       objeto.focus();
  ...
  {   
    if (objeto != "")
    {
       ...
       { 
             ...
             document.all.sectionBenficioTributario.style.display = 'none';
             document.all.sectionBeneTributCuentTercTitu.style.display = 'none';
       } 
       ...
       { 
             ...
             document.all.section6.style.display = 'none';
             document.all.sectionBeneTributCuentTercTitu.style.display = 'none';
             document.all.sectionBenficioTributario.style.display = 'block';
       } 
       if(objeto.substring(2,8)=='STA004')
       { 
             document.all.section5.style.display = 'none';
             document.all.section6.style.display = 'none';
             document.all.sectionBenficioTributario.style.display = 'none';
             document.all.sectionBeneTributCuentTercTitu.style.display = 'block';
       } 
   return true;
  } 
 return false;
